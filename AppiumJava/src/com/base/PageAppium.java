...
import static com.base.InitAppium.appPackage;
...
public class PageAppium {
    ...
    public AndroidElement findByNameEnCh(String name,String nameCh, String desc) {
    	return findElementByEnCh(By.name(name), By.name(nameCh), desc);
    }
     ...
     * @return
     */
    public AndroidElement findElementByEnCh(By by,By byEn,String str){
    	AndroidElement aElement = null;
    	try {
    		if (driver != null) {
    			aElement = (AndroidElement) driver.findElement(by);
    		} else {
    			print("driver为空");
    		}
    	} catch (NoSuchElementException e) {
    	...
    	if (aElement == null) {
    		try {
    			if (driver != null) {
    				aElement = (AndroidElement) driver.findElement(byEn);
    			} else {
    				print("driver为空");
    			}
			} catch (NoSuchElementException e) {
     ...
     * By.id(id)
     */
    public boolean waitForExists(String byId, int time) {
    	Object AndroidElement = null;
        try {
        	AndroidElement = new AndroidDriverWait(driver, time)
                    .until(new ExpectedCondition<AndroidElement>() {
                        @Override
                        public AndroidElement apply(AndroidDriver androidDriver) {
                            return (AndroidElement) androidDriver.findElement(By.id(byId));
                        }
                    });
        } catch (TimeoutException e) {
        	AndroidElement = null;
