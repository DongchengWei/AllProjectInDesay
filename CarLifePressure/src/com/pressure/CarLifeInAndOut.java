...
public class CarLifeInAndOut extends UiAutomatorTestCase {
	//调用自动运行cmd命令
	public static void main(String[] args) throws IOException {  
		 
		RunTestCase runTestCase=new RunTestCase("CarLifeInAndOut",  
				 "com.pressure.CarLifeInAndOut", "", "2");  
		...
		try{
			...
			try {
				new UiObject(new UiSelector()
						.resourceId("com.android.launcher:id/ts_home_button_item_apps_id")).click();
				new UiObject(new UiSelector().
						resourceId("com.android.launcher:id/ts_app_name").text("CarLife")).click();
				new UiObject(new UiSelector().className("android.widget.TextView").text("帮助")).click();
				getUiDevice().pressBack();
				
			} catch (UiObjectNotFoundException e) {
				logAndPrint("CarLifeInAndOut Test fail:UiObjectNotFoundException");
			}
