...
public class MediaPage extends UiAutomatorTestCase {
	...
	//ipod
	UiObject ipodSourceObj = new UiObject(new UiSelector()
			.resourceId("com.thundersoft.mediaplayer:id/ts_media_source_button_item_device_ipod"));
	//aux
	UiObject auxSourceObj = new UiObject(new UiSelector()
			.resourceId("com.thundersoft.mediaplayer:id/ts_media_source_button_item_device_aux"));
	//本地源
	...
	UiObject videoObj = new UiObject(new UiSelector()//视频
			.resourceId("com.thundersoft.mediaplayer:id/ts_media_button_item_movie_id"));
	UiObject imageObj = new UiObject(new UiSelector()//视频
			.resourceId("com.thundersoft.mediaplayer:id/ts_media_button_item_picture_id"));
	...
	//下一曲
	UiObject nextVideoObj = new UiObject(new UiSelector()
			.resourceId("com.thundersoft.mediaplayer:id/iv_ts_media_btn_next_video"));
...
//		}
	 ...
	 * @Date 2017-06-07
	 */
	public boolean intoIpodMusic() throws UiObjectNotFoundException {
		Utils.getCurrentMethodName();
		boolean intoOk = false;
		
		if (mediaTitleObj.exists()) {
			if (SourceJudge() == IPODTITLE) {
				intoOk = true;
			} else {
				if (mediasourceObj.click()) {
					if (ipodSourceObj.click()) {
						if (SourceJudge() == IPODTITLE) {
							intoOk = true;
						}
					}
	 ...
	 * @Date 2017-06-08
	 */
	public boolean intoAuxMusic() throws UiObjectNotFoundException {
		Utils.getCurrentMethodName();
		boolean intoOk = false;
		
		if (mediaTitleObj.exists()) {
			if (SourceJudge() == AUXTITLE) {
				intoOk = true;
			} else {
				if (mediasourceObj.click()) {
					if (ipodSourceObj.click()) {
	...
	public boolean clickNextButton() throws UiObjectNotFoundException {
		...
		if (nextButtonObj.exists()) {
			if (nextButtonObj.click()) {
				...
				Utils.logPrint("nextButtonObj click fail");
			}
		} else {
			Utils.logPrint("nextButtonObj not exists");
		}
	 ...
	 * @Date 2017-06-20
	 */
	public boolean clickNextImageBtn() {
		boolean isOk = false;
		
		
		if (UiDevice.getInstance().click(1218, 378)) {
			isOk = true;
			Utils.logPrint("nextImage click ok");
		} else {
			Utils.logPrint("nextImage click fail");
		}
	 ...
	 * @Date 2017-06-20
	 */
	public boolean clickNextMusicPot() {
		boolean isOk = false;
		
		
		if (UiDevice.getInstance().click(885, 728)) {
			isOk = true;
			Utils.logPrint("clickNextMusicPot click ok");
		} else {
			Utils.logPrint("clickNextMusicPot click fail");
		}
	 ...
	 * 885,745
	 * */
	public boolean clickNextVideoBtn() throws UiObjectNotFoundException {
		
		boolean isOk = false;
		
		
		if (UiDevice.getInstance().click(885, 745)) {
			isOk = true;
			Utils.logPrint("nextVideoObj click ok");
		} else {
			Utils.logPrint("nextVideoObj click fail");
		}
	...
	public boolean intoSdMusic() throws UiObjectNotFoundException {
		...
		if (backOk && mediasourceObj.click()) {
			if (sdSourceObj.isEnabled()) {//usb1已插上则默认进入usb1
				sdSourceObj.click();
				if (musicObj.click()) {//音乐视频图片三个选项选择音乐
					if (musicStatusbarObj.click()) {//点击正在播放的曲目状态栏
				...
				Utils.logPrint("Sdcard is disable!");
			}
		}
	...
	public boolean intoUsb1Music() throws UiObjectNotFoundException {
		...
		if (backOk && mediasourceObj.click()) {
			if (usb1SourceObj.isEnabled()) {//usb1已插上则默认进入usb1
				...
				Utils.logPrint("Usb1 is disable!");
			}
		}
		return intoOk;
	}
	...
	public boolean intoUsb2Music() throws UiObjectNotFoundException {
		...
		if (backOk && mediasourceObj.click()) {
			if (usb2SourceObj.isEnabled()) {//usb1已插上则默认进入usb1
				...
				Utils.logPrint("Usb2 is disable!");
			}
		}
		return intoOk;
	}
	...
	public boolean id3Check() throws UiObjectNotFoundException {
		boolean isID3Ok = true;
		
		String titleStr = getMusicTitle();
		String artistStr = getMusicArtistText();
		String albumStr = getMusicAlbumText();
		Utils.logPrint("Title:" + titleStr + ", Artst:" + artistStr + ", Album:" + albumStr);
		if (titleStr == null || titleStr.equals("") || artistStr == null || artistStr.equals("") || albumStr == null || albumStr.equals("")) {
			isID3Ok = false;
		} 
	 ...
	 * 	public static final int USB1SOURCE = 2;
	 * 	public static final int USB2SOURCE = 3;
	 * 	public static final int SDSOURCE = 4;
	 * @return
	 * @Date 2017-06-20
	 */
	public static final int LOCALSOURCE = 1;
	...
	public static final int USB2SOURCE = 3;
	public static final int SDSOURCE = 4;
	public boolean selectSource(int sourceInt) throws UiObjectNotFoundException {
		//TODO
		boolean isOK = false;
//		if (mediaTitleObj.waitForExists(3000)) {
//		   Utils.logPrint("mediaTitleObj.waitForExists(3000)");
//		}
		
		switch (sourceInt) {
		case LOCALSOURCE:
			if (localSourceObj.isEnabled()) {
				localSourceObj.click();
				if (mediaTitleObj.waitForExists(3000)) {
					isOK = true;
				} else {
					Utils.logPrint("localSourceObj.click() fail");
				}
			} else {
				Utils.logPrint("localsource is disable");
			}
			break;
		case USB1SOURCE:
			if (usb1SourceObj.isEnabled()) {
				usb1SourceObj.click();
				if (mediaTitleObj.waitForExists(3000)) {
					isOK = true;
				} else {
					Utils.logPrint("usb1SourceObj.click() fail");
				}
			} else {
				Utils.logPrint("USB1 is disable");
			}
			break;
		case USB2SOURCE:
			if (usb2SourceObj.isEnabled()) {
				usb2SourceObj.click();
				if (mediaTitleObj.waitForExists(3000)) {
					isOK = true;
				} else {
					Utils.logPrint("usb2SourceObj.click() fail");
				}
			} else {
				Utils.logPrint("USB2 is disable");
			}
			break;
		case SDSOURCE:
			if (sdSourceObj.isEnabled()) {
				sdSourceObj.click();
				if (mediaTitleObj.waitForExists(3000)) {
					isOK = true;
				} else {
					Utils.logPrint("sdSourceObj.click() fail");
				}
			} else {
				Utils.logPrint("SDCARD is disable");
			}
	...
	UiObject firstMvObj = new UiObject(new UiSelector().resourceId("com.thundersoft.mediaplayer:id/tv_kindlabel"));
	public boolean selectSourceType(int sourceType) throws UiObjectNotFoundException {
		boolean isOk = false;
		...
		case TYPEMUSIC:
			if (musicObj.click()) {
				if (musicStatusbarObj.click()) {//点击正在播放的曲目状态栏
					...
					Utils.logPrint("musicStatusbarObj.click() fail");
				}
			} else {
				Utils.logPrint("musicObj.click() fail");
			}
			break;
		case TYPEVIDEO:
			if (videoObj.click()) {
				if (firstMvObj.exists()) {
					if (firstMvObj.click()) {
						isOk = true;
					} else {
						Utils.logPrint("firstMvObj.click() fail");
					}
				} else {
					Utils.logPrint("firstMvObj.exists() not");
				}
			} else {
				Utils.logPrint("videoObj.click() fail");
			}
			break;
		case TYPEIMAGE:
			if (imageObj.click()) {
				if (firstMvObj.exists()) {
					if (firstMvObj.click()) {
						...
						Utils.logPrint("firstImageObj.click() fail");
					}
				} else {
					Utils.logPrint("firstImageObj.exists() not");
				}
			} else {
				Utils.logPrint("videoObj.click() fail");
			}
	 ...
	 * @Date 2017-06-20
	 */
	public boolean intoSourceSelect() throws UiObjectNotFoundException {
		Utils.getCurrentMethodName();
		boolean intoOk = false;
		...
		if (mediasourceObj.exists()) {
			if (mediasourceObj.click()) {
				...
				Utils.logPrint("mediasourceObj.click() fail");
			}
		} else {
			UiDevice.getInstance().pressBack();
			if (mediasourceObj.exists()) {
				if (mediasourceObj.click()) {
					...
					Utils.logPrint("press back then mediasourceObj.click() fail");
				}
			} else {
				Utils.logPrint("press back mediasourceObj.exists() fail");
			}
		}
		return intoOk;
	}
