...
public class CarLifePage extends UiAutomatorTestCase {
	...
	int yPlay = 420;
	int widthPlay = 70;
	int heighPlayt = 107;//589, 405, 108, 135
	...
	public boolean isPlaying() throws FileNotFoundException {
		...
		if (carLifeMusicPlayingFile.exists()) {
			...
			if (UiDevice.getInstance().takeScreenshot(carLifeMusicPageFile)) {
				...
				//获取播放页面的子截图（播放按钮）
				Bitmap pauseButton = CaptrueUtil
						.getSubImage(CARLIFEMUSICPLAYING, xPlay, yPlay, widthPlay, heighPlayt);//589, 405, 108, 135
				
				//截图后的按钮
				Bitmap pauseButtonNow = CaptrueUtil
						.getSubImage(CARLIFEMUSICPAGE, xPlay, yPlay, widthPlay, heighPlayt);//589, 405, 108, 135
		...
		if (carLifeMusicPauseFile.exists()) {
			...
			if (UiDevice.getInstance().takeScreenshot(carLifeMusicPageFile)) {
				...
				Bitmap playingButton = CaptrueUtil
						.getSubImage(CARLIFEMUSICPAUSE, xPlay, yPlay, widthPlay, heighPlayt);
				//截图后的按钮
				Bitmap playingButtonNow = CaptrueUtil
						.getSubImage(CARLIFEMUSICPAGE, xPlay, yPlay, widthPlay, heighPlayt);
...
//		return CaptrueUtil.getSubImage(path, 150, 100, 145, 160);
		return CaptrueUtil.getSubImage(path, 153, 103, 145, 48);
	}
	 ...
	 * @Date 2017-06-06
	 */
	public Bitmap getAlbumBitmap(String path) throws FileNotFoundException {
		return CaptrueUtil.getSubImage(path, 153, 174, 186, 35);
	}
	 ...
	 * @Date 2017-06-06
	 */
	public Bitmap getArtistBitmap(String path) throws FileNotFoundException {
		return CaptrueUtil.getSubImage(path, 153, 226, 186, 35);
	}
