...
public class RunTestCase {
	...
	private static String jar_name = "AutoTest";
	private static String test_class = "com.autotest.PressureTest";
	private static String test_name = "AutoTest";

	private static String workspace_path;
	
	private static String outputlogPathStr;
	...
	public static void main(String[] args) {
		RunTestCase runCase = new RunTestCase();//获取工作空间目录
	...
	// 原生工程运行步骤,pathStr为日志路径
	public String runUiautomator() {
		
		String startTestTimeStr = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		File outputLogsFile = new File("d:/android/AutoTestLogs" + "/" + startTestTimeStr);
		if (! outputLogsFile.exists()) {
			outputLogsFile.mkdirs();
		}
		outputlogPathStr = "d:/android/AutoTestLogs" + "/" + startTestTimeStr;//目录
		...
		return outputlogPathStr;//返回日志路径
	}
	...
	public void execCmd(String cmd) {
		...
		try {
			...
			FileWriter writer = new FileWriter(outputlogPathStr + "/UaLogs.txt", true);//写到文件
